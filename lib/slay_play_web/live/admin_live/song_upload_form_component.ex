defmodule SlayPlayWeb.AdminLive.SongUploadFormComponent do
  use SlayPlayWeb, :live_component

  alias SlayPlay.{Player, MP3Stat}
  alias SlayPlayWeb.AdminLive.SongEntryComponent
  ...
  def handle_event("save", %{"songs" => params}, socket) do
    ...
    if pending_stats?(socket) do
      ...
      case Player.import_songs(changesets, &consume_entry(socket, &1, &2)) do
        {:ok, songs} ->
          {:noreply,
           socket
  ...
  defp apply_params(socket, params, action) when action in [:validate, :insert] do
    Enum.reduce(params, socket, fn {ref, song_params}, acc ->
      new_changeset =
        acc
        |> get_changeset(ref)
        |> Player.change_song(song_params)
  ...
  defp put_new_changeset(socket, entry) do
    cond do
      ...
      true ->
        attrs = Player.parse_file_name(entry.client_name)
        changeset = Player.change_song(%Player.Song{}, attrs)
  ...
  defp async_calculate_duration(socket, %Phoenix.LiveView.UploadEntry{} = entry) do
    ...
    consume_uploaded_entry(socket, entry, fn %{path: path} ->
      Task.Supervisor.start_child(SlayPlay.TaskSupervisor, fn ->
        send_update(lv, __MODULE__,
          id: socket.assigns.id,
          action: {:duration, entry.ref, MP3Stat.parse(path)}
        )
      end)
  ...
  defp put_stats(socket, entry_ref, %MP3Stat{} = stat) do
    if changeset = get_changeset(socket, entry_ref) do
      case Player.put_stats(changeset, stat) do
        {:ok, new_changeset} ->
